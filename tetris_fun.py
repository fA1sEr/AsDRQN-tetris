...
class GameState:
    ...
    def get_state(self):
        ...
        x_t = cv2.cvtColor(cv2.resize(image_data, STATE_SHAPE), cv2.COLOR_BGR2GRAY)
    ...
    def frame_step(self,input):
        ...
        if not self.isValidPosition(adjY=1):
            ...
            if cleared > 0:
                ...
                print('cleared: {}, score: {}\n'.format(str(cleared),str(self.score)))
                reward = 2**cleared
            ###
            ...
            changeHeight = self.height - self.getHeight()
            changeHoles = self.holes - self.getHoles()
            changeContours = self.contours - self.getContours()
            
            reward += self.lines * 0.76 + (0.51 * changeHeight + 0.36 * changeHoles + 0.18 * changeContours)
        ...
        return x_t, reward, terminal, cleared
    ...
    def getHoles(self):
        holes = 0
        for col_index, colomn in enumerate(self.board):
            for row_index, square in enumerate(colomn):
                if row_index == 0:
                    continue
                if square == '.' and self.board[col_index][row_index-1] != '.':
                    holes += self.get_holes_at_square(self.board, row_index, col_index)

        return holes


    def getColomnHeight(self,col_index):
        stack_height = 0
        for i in range(0, BOARDHEIGHT-1):
            blank_row = True
            if self.board[col_index][i] != '.':
                blank_row = False
            if not blank_row:
                stack_height = BOARDHEIGHT - i - 1
                break
        return stack_height

    def getContours(self):
        total_contours = 0
        column_height = self.getColomnHeight(0)
        for col_idx in range(1, len(self.board)):
            next_column_height = self.getColomnHeight(col_idx)
